                ...
                resp.forEach((u) => {
                    if (u.extra_info) {
                        u.extra_info.forEach((extra) => {
                            if (!extras[extra.title]) {
                                extras[extra.title] = {};
                            }
                            if (Array.isArray(extra.value)) {
                                for (let i = 0; i < extra.value.length; i++) {
                                    let e = extra.value[i];
                                    if (!extras[extra.title][e]) {
                                        extras[extra.title][e] = 0;
                                    }
                                    extras[extra.title][e]++;
                                }
                            } else {
                                if (!extras[extra.title][extra.value]) {
                                    extras[extra.title][extra.value] = 0;
                                }
                    ...
                    if (u.email) {
                        ...
                        if (domain.length > 1) {
                            if (!mail_domain[domain[1]]) {
                                mail_domain[domain[1]] = 0;
                            }
                            mail_domain[domain[1]]++;
                        }
                    }
                    if (!statuses[u.status]) {
                        statuses[u.status] = 0;
                    }
                    statuses[u.status]++;
                });
